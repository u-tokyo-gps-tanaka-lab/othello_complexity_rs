...
#[inline] fn sh_n(b: u64) -> u64 { b << 8 }
#[inline] fn sh_s(b: u64) -> u64 { b >> 8 }
#[inline] fn sh_e(b: u64) -> u64 { (b << 1) & !FILE_A }                 // H→A への横溢れを除去
#[inline] fn sh_w(b: u64) -> u64 { (b >> 1) & !FILE_H }                 // A→H への横溢れを除去
#[inline] fn sh_ne(b: u64) -> u64 { (b & !FILE_H) << 9 }                // 出発がH列ならNE不可
#[inline] fn sh_nw(b: u64) -> u64 { (b & !FILE_A) << 7 }                // 出発がA列ならNW不可
#[inline] fn sh_se(b: u64) -> u64 { (b & !FILE_H) >> 7 }                // 出発がH列ならSE不可
#[inline] fn sh_sw(b: u64) -> u64 { (b & !FILE_A) >> 9 }                // 出発がA列ならSW不可
